<template>
    <form class="nftmax-wc__form-main" action="/" method="post">
        <div v-if="showLoader" id="loader-out">
            <div id="loader-container">
                <p id="loading-text">Cargando</p>
            </div>
        </div>
        <div class="form-group">
            <label class="nftmax-wc__form-label">Número de CIP</label>
            <div class="form-group__input">
                ...
                <input v-model="cip" class="nftmax-wc__form-input" type="email" name="email" placeholder="Ingrese su número de CIP" required="required">
            </div>
        </div>
        <div class="form-group">
            <label class="nftmax-wc__form-label">Token Digital SIGCP</label>
            <div class="form-group__input">
                <div class="form-group__input">
                    ...
                    <input v-model="otp" class="nftmax-wc__form-input" placeholder="● ● ● ● ● ●" id="password-field" type="password" name="password" maxlength="8" required="required">
                </div>
            </div>
        </div>
        <div class="form-group">
            <div class="nftmax-wc__check-inline">
                <div class="nftmax-wc__forgot">
                    <router-link to="/forgot-password" class="forgot-pass">Olvidaste tu contraseña?</router-link>
                </div>

            </div>
            <div class="form-group">
                <div class="nftmax-wc__button">
                    <button @click="submitForm" class="ntfmax-wc__btn" type="button">Iniciar Sesión</button>
                </div>
            </div>
            <div class="text-center mt-2">
                <p class="text-success">Recuerda que el Token Digital lo obtienes desde la aplicación <a href="">SIGCP APP</a></p>
            </div>
        </div>
        <div class="form-group">
            <div class="nftmax-wc__bottom">
                <p class="nftmax-wc__text">Manual de uso del Token Digital PNP</p>
                <router-link to="/signup"> Descargar Instructivo</router-link>
            </div>
        </div>
    </form>
</template>
